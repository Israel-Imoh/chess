...
const ChessGame = () => {
  ...
  useEffect(() => {
    const updateSize = () => {
      const maxWidth = window.innerWidth - 300; // account for move log
      const maxHeight = window.innerHeight - 150; // account for padding/status/button
      const size = Math.min(maxWidth, maxHeight, 600);
      setBoardSize(Math.max(size, 300)); // minimum 300px
    };
